...
//				return (float)(1.0);
	...
	public int[] getAngleOfCrossingHistogram(int numBins, boolean inverted) {
		...
		if (dimension1 > dimension2)
			return model.getAxisPair(dimension2, dimension1, parallelDisplay).getAngleOfCrossingHistogram(numBins, inverted);
		else {
	...
	public ValuePair getAngleOfCrossingMedian(int numBins) {
		...
		if (anglesOfCrossingsMedian == null) {
			if (dimension1 > dimension2)
				anglesOfCrossingsMedian = model.getAxisPair(dimension2, dimension1, parallelDisplay).getAngleOfCrossingMedian(numBins);
			else {
	...
	public Point2D.Float getParallelism(int numBins) {
		...
		if (parallelism == null) {
			if (dimension1 > dimension2)
				parallelismMetric = model.getAxisPair(dimension2, dimension1, parallelDisplay).getParallelism(numBins);
			else {
		...
		if (mutualInformationCached < 0) {
			if (dimension1 > dimension2)
				mutualInformation = model.getAxisPair(dimension2, dimension1,
						parallelDisplay).getMutualInfo(numBins);
			else {
	...
	public ValuePair getDegreeOfOverPlotting(int numBins) {
		...
		if (overplotting == null) {
			if (dimension1 > dimension2)
				overplotting = model.getAxisPair(dimension2, dimension1, parallelDisplay).getDegreeOfOverPlotting(numBins);
			else {
...
//			return new ValuePair(n.getValue()/10, n.getInvertedValue()/10);
	...
	public int[] getDistanceHistogramScatter(int numBins, boolean inverted)  {
		...
		if (dimension1 > dimension2)
			return model.getAxisPair(dimension2, dimension1, parallelDisplay).getDistanceHistogramScatter(numBins, inverted);
		else {
	...
	public int[] getDistanceHistogram(int numBins, boolean inverted)  {
		...
		if (dimension1 > dimension2)
			return model.getAxisPair(dimension2, dimension1, parallelDisplay).getDistanceHistogram(numBins, inverted);
		else {
			if (distanceHistogram == null)
				calculateHistograms(numBins);
